...
| OPH-017 | P1 | DONE | Counter-signal and erasure ledger nodes | Anonymous bursts rotate once per interval and coalesce into one expiring UTC-hour node with accepted/applied/deferred totals plus a typed edge to the runtime field; no request-level or visitor trace is retained |
| OPH-018 | P1 | DONE | Curatorial decision relations | Rationale-required approve/reject/revise transactions atomically append attributed governance nodes/edges; existing provenance import is idempotent and comparison remains browser-only reversible state |
...
| OPH-020 | P2 | READY | Simulated physical bridge contract | Deterministic light/sound adapter validates bounded scores and quiet fallback without real hardware action |
