...
|---|---|---|
| Runtime organism | Aggregate hourly events, labelled field score, bounded three-lure renderer, consequential refusal, candidate composition, human publication gate | OPH-016 adds a timestamped five-state continuity projection with a documented local freshness policy and no claim of current liveness |
...
| 2 | OPH-017 · Counter-signal / erasure ledger nodes | OPH-015 topology; identity-free refractory bound on the current refusal transaction | An anonymous burst rotates the field at most once per declared interval; a refusal creates only a coarse counter-signal node/edge with bounded retention or aggregation and no individual attribution |
