...
test('public surfaces preserve keyboard, motion, contrast, mobile and error-state boundaries', () => {
  ...
  assert.match(publicScript, /article\.href = PUBLIC_WORK_ROUTES\[work\.id\] \|\| '\/studio#works'/)
...
test('the coded quartet couples four public artwork heroes to bounded counter-actions', () => {
  ...
  assert.match(page, /OPHRYS <span>ARTWORKS<\/span>/)
  assert.match(page, /id="enter-work"[\s\S]*Enter artwork/)
  ...
  assert.match(script, /sendStudyEvent\('refusal'\)/)
  assert.match(script, /sendStudyEvent\('artwork_open'\)/)
  assert.match(publicPage, /id="artworks"[\s\S]*class="artwork-hero[^"]*"[\s\S]*data-study-status[\s\S]*Curatorial status unavailable/)
  assert.equal((publicPage.match(/data-study-id="study-/g) || []).length, 4)
  assert.doesNotMatch(publicPage, /<article[^>]+style=/)
  assert.match(publicFile('styles.css'), /\.hero-borrowed-weather\{--hero-accent:#dfff52/)
  assert.match(publicFile('app.js'), /renderStudyStatuses\(state\?\.studyStatuses\)/)
  assert.match(publicFile('app.js'), /\.artwork-heroes'\)\.setAttribute\('aria-busy', 'false'\)/)
  assert.match(publicFile('app.js'), /initializeArtworkHeroes\(\)/)
  assert.match(publicFile('app.js'), /new IntersectionObserver/)
  assert.match(publicFile('app.js'), /pointermove/)
