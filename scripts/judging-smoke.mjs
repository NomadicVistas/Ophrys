...
try {
  ...
  assert.equal(studio.payload.physicalBridge.status, 'simulated')
  assert.equal(studio.payload.physicalBridge.source.revision, refusal.payload.fieldScore.revision)
  assert.equal(studio.payload.physicalBridge.safety.hardwareAction, false)
  assert.equal(studio.payload.physicalBridge.safety.transport, 'none')
  assert.equal(studio.payload.ecosystem.nodeTypeCounts.physicalOutput, 1)
  const physicalOutput = studio.payload.ecosystem.nodes.find(node => node.nodeType === 'physical-output')
  assert.equal(studio.payload.ecosystem.relations.some(relation => relation.fromNodeId === 'runtime-public-field' && relation.toNodeId === physicalOutput.id && relation.kind === 'simulated-as'), true)
  ...
  assert.doesNotMatch(JSON.stringify(studio.payload), /redacted\/judging-route|redacted-fixture-response/)
  const judgingCandidate = studio.payload.artworks.find(work => work.id === 'judge-trace-candidate')
  assert.deepEqual(judgingCandidate.provenance.inputSummary.aggregateEventSummary, [{ kind: 'refusal', count: 1 }])
  assert.equal(Object.hasOwn(studio.payload.cycles[0], 'responseId'), false)
  ...
  assert.match(JSON.stringify(operator.payload), /redacted\/judging-route/)
