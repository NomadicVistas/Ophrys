...
| Reduced motion | The reduced-motion query disables animations and transitions and restores non-animated scrolling. The field-specific stylesheet also stops orbit motion. Each coded study renders a labelled static score and work-specific instruction, runs no continuous animation loop, and keeps its counter-state static instead of promising a restoration that no loop can show. | Source assertions plus `npm run test:accessibility`, which emulates `prefers-reduced-motion: reduce` through Chromium DevTools and proves the animation-frame count settles. |
...
| Mobile | At 720 px and below, primary navigation remains available as a horizontally scrollable 44 px-high row; the education encounter, artwork, telemetry, metric, Studio and Operator grids collapse without a required horizontal page scroll. | Responsive source assertion, earlier Chromium smoke at 390 × 844, and a repeatable four-study reduced-motion smoke at 320 × 700 with zero horizontal overflow. |
...
The coded quartet uses native buttons and links for entry, counter-action, notes, optional sound and sequential navigation. Each canvas has a work-specific visual description, while the notes panel exposes the full encounter, question, counter-reading and data boundary as text. The renderer caps device pixel ratio, pauses when the page is hidden and responds if the device motion preference changes. Reduced-motion mode exposes a static score label, keeps the work instruction, Studio link and counter-action visible, and does not store the preference. `Choir of Almost` synthesises optional local sound only after an explicit action and never requests microphone access. `Afterimage Commons` retains traces only in page memory and visibly expires them.

`npm run test:accessibility` requires a local Chromium executable (or `CHROMIUM_BIN`) but no browser library. It starts an ephemeral in-memory Ophrys server, emulates a 320 × 700 CSS-pixel viewport and reduced motion, checks all four studies, and removes its temporary browser profile. It performs no external request, publication, hardware action or persistent visitor write.
